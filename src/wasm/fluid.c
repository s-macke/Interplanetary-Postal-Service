...
void SetEdgesEqual(float **x)
{
    ...
    x[M+1][0]           = 0.5 * (x[M+1][1] + x[M][0]);
    x[0][N+1]           = 0.5 * (x[0][N]   + x[1][N + 1]);
    x[M+1][N+1]         = 0.5 * (x[M+1][N] + x[M][N + 1]);
}
    ...
    for(int j=1; j<M+1; j++)
    {
        ...
        for(int i=1; i<N+1; i++)
        {
            ...
            if (cellRow[i] == BOUNDARY)
            {
                ...
                if (n != 0) xRow[i] = add / (float)n; else xRow[i] = 0.;
            }
        }
    }
    SetEdgesEqual(x);
...
{    
    for(int j=1; j<M+1; j++)
    {
        ...
        for(int i=1; i<N+1; i++)
        {
            if (cellRow[i]) xRow[i] = 0.; // if boundary
        }
    }
    SetEdgesEqual(x);
}
...
void Advect(float **d0, float **d, float dt)
{
    ...
    for (int j = 1; j<= M; j++)
    {
        for (int i = 1; i <= N; i++)
        {
            ...
            if (x < 0.5) x = 0.5; else if (x > Wp5) x = Wp5;
            int i0 = x;
            int i1 = i0 + 1;
            if (y < 0.5) y = 0.5; else if (y > Hp5) y = Hp5;
            int j0 = y;
            int j1 = j0 + 1;
            float s1 = x   - i0;
            float s0 = 1.  - s1;
    ...
    for (int k=0 ; k<iterations; k++)
    {
        ...
        for (int j=1 ; j<=M; j++)
        {
            ...
            for (int i=1; i<=N; i++)
            {
                ...
                currentRow[i] = (currentRow0[i] + a * (currentRow[i-1] + currentRow[i+1] + lastRow[i] + nextRow[i])) * invC;
...
void Project()
{
    ...
    float h = -0.00276213586; // well, just to prevent usage of sqrtf
...
void __attribute__((export_name("_FixCells"))) FixCells()
{
    ...
    for (int i = 1; i<= N; i++)
    {
        ...
        if (cell[j][i] 
                ...
                && cell[j+1][i]==FLUID
                && cell[j-1][i-1]==FLUID
                && cell[j-1][i+1]==FLUID) cell[j][i] = FLUID;
...
void FluidStep(float dt)
{
    ...
    // switch arrays
    temp = vx; vx = prevvx; prevvx = temp;
    temp = vy; vy = prevvy; prevvy = temp;

    Project();

    // switch arrays
    temp = density1; density1 = prevdensity1; prevdensity1 = temp;
    temp = density2; density2 = prevdensity2; prevdensity2 = temp;
    temp = density3; density3 = prevdensity3; prevdensity3 = temp;
